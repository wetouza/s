...
# ═══════════════════════════════════════════════════════════════════════
LANGUAGE_NAMES = {"auto": "Auto", "en": "English", "ru": "Русский"}
I18N = {
    "en": {"language":"Language","auto":"Auto","english":"English","russian":"Russian","favorites":"Favorites","settings":"Settings","search_placeholder":"Search tags. − to exclude, rating: score: sort: for filters","save_current":"Save current","load_more":"Load more","drop_last":"Drop the last tag","reset_filters":"Reset filters","review_blacklist":"Review blacklist","retry":"Retry","account_settings":"Account settings","favorite":"Favorite","favorited":"Favorited","hide_info":"Hide info","show_info":"Show info"},
    "ru": {"language":"Язык","auto":"Авто","english":"Английский","russian":"Русский","favorites":"Избранное","settings":"Настройки","search_placeholder":"Поиск тегов. − исключить, rating: score: sort: для фильтров","save_current":"Сохранить","load_more":"Загрузить ещё","drop_last":"Убрать последний тег","reset_filters":"Сбросить фильтры","review_blacklist":"Проверить чёрный список","retry":"Повторить","account_settings":"Настройки аккаунта","favorite":"В избранное","favorited":"В избранном","hide_info":"Скрыть сведения","show_info":"Показать сведения"}
}
...
/* ═══════════ localization ═══════════ */
const LANG = { en: {"language":"Language","auto":"Auto","english":"English","russian":"Russian","favorites":"Favorites","settings":"Settings","search_placeholder":"Search tags. − to exclude, rating: score: sort: for filters","save_current":"Save current","load_more":"Load more","drop_last":"Drop the last tag","reset_filters":"Reset filters","review_blacklist":"Review blacklist","retry":"Retry","account_settings":"Account settings","favorite":"Favorite","favorited":"Favorited","hide_info":"Hide info","show_info":"Show info"}, ru: {"language":"Язык","auto":"Авто","english":"Английский","russian":"Русский","favorites":"Избранное","settings":"Настройки","search_placeholder":"Поиск тегов. − исключить, rating: score: sort: для фильтров","save_current":"Сохранить","load_more":"Загрузить ещё","drop_last":"Убрать последний тег","reset_filters":"Сбросить фильтры","review_blacklist":"Проверить чёрный список","retry":"Повторить","account_settings":"Настройки аккаунта","favorite":"В избранное","favorited":"В избранном","hide_info":"Скрыть сведения","show_info":"Показать сведения"} };
function browserLanguage(){ return (navigator.language || "en").toLowerCase().startsWith("ru") ? "ru" : "en"; }
function language(){ return S.cfg && S.cfg.language !== "auto" ? S.cfg.language : browserLanguage(); }
function t(key){ return (LANG[language()] || LANG.en)[key] || LANG.en[key] || key; }
function localize(){
  document.documentElement.lang = language();
  $("#q").placeholder = t("search_placeholder");
  $("#favBtn .lbl").textContent = t("favorites"); $("#setBtn .lbl").textContent = t("settings");
  $("#saveSearch").lastChild.textContent = " " + t("save_current"); $("#loadMore span").textContent = t("load_more");
  $("#dropLast").textContent = t("drop_last"); $("#clearAll").textContent = t("reset_filters"); $("#openBl").textContent = t("review_blacklist");
  $("#retryBtn").textContent = t("retry"); $("#openKey").textContent = t("account_settings");
  if($("#sideToggle")) $("#sideToggle").textContent = $("#vwrap").classList.contains("solo") ? t("show_info") : t("hide_info");
  if($("#vFavLbl")) $("#vFavLbl").textContent = S.posts[S.cur] && S.favIds.has(S.posts[S.cur].id) ? t("favorited") : t("favorite");
  const sel=$("#language"); if(sel){ sel.value=S.cfg.language||"auto"; sel.options[0].textContent=t("auto"); sel.options[1].textContent=t("english"); sel.options[2].textContent=t("russian"); }
}
...
function paintFavUI(id) {
  ...
  if (viewer.open && S.posts[S.cur] && S.posts[S.cur].id === id) {
    $("#vFavLbl").textContent = on ? t("favorited") : t("favorite");
...
(async function boot() {
  try {
    ...
    syncSettingsUI();
    localize();
    $("#language").addEventListener("change", e => { saveCfg({ language: e.target.value }); setTimeout(localize, 300); });
  ...
  <div class="body">
    ...
    <section class="tabpane on" data-pane="appearance">
      <div class="group">
        ...
        <div class="field"><div><div class="t">Language</div><div class="d">Choose the interface language</div></div>
          <select class="inp" id="language" style="width:auto;min-width:140px;height:32px"><option value="auto">Auto</option><option value="en">English</option><option value="ru">Русский</option></select></div>
      </div>
      <div class="group">
