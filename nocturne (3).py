...
<header id="topbar">
  <div class="bar row1">
    <div class="brand"><span class="mark"><svg><use href="#i-moon"/></svg></span><span data-i18n="app">Nocturne</span><small>2.3</small></div>
    <div id="searchwrap">
...
window.addEventListener("unhandledrejection", e => {
  if (e.reason && e.reason.api) return;
  console.warn("Nocturne UI request failed", e.reason);
});
...
async function fetchAc(text) {
  ...
  try {
    ...
    if (!ctl.signal.aborted && !acBox.hidden && qInput.value.trim().replace(/^-/, "") && normTag(qInput.value.replace(/^-/, "")) === term) renderAcRows(rows);
